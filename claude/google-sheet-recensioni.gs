 ...
 * 1. Apri il foglio Google da usare (già pronto, con l'intestazione
 *    corretta — se non ce l'hai ancora, creane uno nuovo e vuoto su
 ...
 * serve cambiare nulla nel sito.
 *
 * COME SCEGLIERE COSA VA SUL SITO: non serve più segnare "Pubblica" nel
 * foglio (quella colonna resta lì ma il pannello admin non la guarda
 * più) — ogni recensione con nome e testo viene importata in automatico.
 * La colonna "In home (SI/NO)" resta come suggerimento di partenza per
 * l'homepage, ma la scelta vera e propria — riga per riga, "mostra
 * anche in homepage" sì o no — la fai comodamente nel pannello admin
 * (sezione Recensioni), dopo aver importato con «Importa dal foglio».
 */
...
function salvaRecensione(dati) {
  // Sempre la PRIMA scheda del foglio (per posizione, non "quella che
  // hai aperta per ultima"): così anche se in futuro aggiungi altre
  // schede (come "Contatti"), le recensioni finiscono sempre nello
  // stesso posto, indipendentemente da cosa stavi guardando nel browser.
