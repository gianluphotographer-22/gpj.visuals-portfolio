...
(function (global) {
  ...
  // Capitoli — usati da pagine come Cuore Biancorosso o T'Immagini, che
  // hanno un indice "I CAPITOLI" scritto a mano: qui rendiamo un nuovo
  // capitolo aggiunto dal pannello con la STESSA identica struttura
  // HTML di quelli scritti a mano, così le CSS della pagina (che non
  // sanno nulla del pannello) lo trattano esattamente allo stesso modo.
  function chapterAnchor(s, num) {
    return slugify(s.title) || s.id || ('capitolo-' + num);
  }

  // Assegna il numero progressivo ai capitoli di una lista di sezioni
  // (raccolte + capitoli mescolati, nell'ordine in cui sono salvati),
  // continuando dal numero di capitoli già scritti a mano nella pagina
  // (startAt). Le sezioni che non sono capitoli restano invariate.
  function numberChapters(sections, startAt) {
    var n = startAt || 0;
    (sections || []).forEach(function (s) {
      if (s.type === 'chapter') { n += 1; s.number = n; }
    });
    return sections;
  }

  function chapterHtml(s, srcFor) {
    var num = s.number ? String(s.number) : '1';
    if (num.length < 2) num = '0' + num;
    var anchor = chapterAnchor(s, num);
    var titleUp = String(s.title || '').toUpperCase();
    return '<section class="gallery-section match-chapter" id="' + esc(anchor) + '">' +
      '<div class="container">' +
      '<div class="eyebrow red reveal">' + esc(num) + (s.eyebrow ? ' — ' + esc(s.eyebrow) : '') + '</div>' +
      (titleUp ? '<h2 class="gallery-title reveal">' + esc(titleUp) + '</h2>' : '') +
      (s.note ? '<p class="match-note reveal">' + esc(s.note).replace(/\n/g, '<br>') + '</p>' : '') +
  ...
  function sectionHtml(s, opts) {
    ...
    if (s.type === 'chapter') return chapterHtml(s, srcFor);
