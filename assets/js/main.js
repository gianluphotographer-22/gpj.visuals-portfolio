...
/* Apertura sempre dall'alto: alcuni browser (soprattutto su telefono)
   ripristinano da soli la posizione di scroll dell'ultima visita,
   anche su quella che sembra un'apertura "nuova" del sito (tornando
   indietro, riaprendo una scheda rimasta in background...). Qui lo
   disattiviamo e, se il link non punta a una sezione precisa (niente
   #ancora nell'indirizzo), riportiamo la pagina in cima appena parte. */
(function () {
...
/* Recensioni: legge dati/recensioni-generali.js (window.GPJ_REVIEWS,
   scritto dal pannello admin — importazione dal Google Sheet o script
   Excel) e riempie due griglie diverse, se presenti in pagina:
     - #home-reviews-grid  (index.html)     solo le "pinnate"
     - #community-reviews-grid (recensioni.html)  tutte quelle valide
   Così sia la homepage sia la pagina Recensioni restano sempre
   allineate a quello che gestisci dall'admin, senza dover più
   rigenerare a mano nessuna pagina. */
(function () {
  const usabili = (window.GPJ_REVIEWS || []).filter(r => r && r.nome && r.testo);
  if (!usabili.length) return;
  ...
  function cardHtml(r, avatarHtml) {
    ...
    return (
      '<article class="review-card reveal">' +
        '<div class="review-header">' + avatarHtml +
  ...
  // Homepage: avatar inline (qui non c'è uno stile di pagina dedicato
  // alla lettera-avatar, quindi lo si costruisce a mano com'era già).
  function avatarInline(r) {
  ...
  // Pagina Recensioni: qui esiste già la classe .review-avatar-lettera
  // (definita nello style della pagina), la stessa usata prima a mano.
  function avatarClasse(r) {
    const iniziale = esc(r.nome.trim().slice(0, 1).toUpperCase());
    return r.avatar
      ? '<img loading="lazy" decoding="async" class="review-avatar" src="' + esc(r.avatar) + '" alt="' + esc(r.nome) + '">'
      : '<div class="review-avatar review-avatar-lettera" aria-hidden="true">' + iniziale + "</div>";
  }

  const homeGrid = document.getElementById("home-reviews-grid");
  if (homeGrid) {
    const pinnate = usabili.filter(r => r.pinned);
    if (pinnate.length) homeGrid.innerHTML = pinnate.map(r => cardHtml(r, avatarInline(r))).join("");
  }

  const communityGrid = document.getElementById("community-reviews-grid");
  if (communityGrid) {
    communityGrid.innerHTML = usabili.map(r => cardHtml(r, avatarClasse(r))).join("");
  }
})();
...
/* Form contatti e recensioni — invio via Web3Forms, senza ricaricare la
   ...
   Ogni form (recensioni, id="review-form", e contatti, id="contact-form")
   manda in più una copia dei dati anche a un Google Sheet, tramite un
   piccolo script Google Apps Script pubblicato come "app web" — vedi
   GUIDA-ADMIN.txt, punto 12, per come crearlo. Le recensioni finiscono
   nel foglio principale come sempre, i messaggi di contatto in una
   scheda separata "Contatti" (creata da sola al primo invio). Finché
   SHEET_WEBHOOK_URL resta vuoto questo invio in più è semplicemente
   saltato, e tutto continua a funzionare come prima (il messaggio arriva
   comunque via email da Web3Forms). */
(function () {
  var SHEET_WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbzdYyJYyjXopYYYt39tqK37ae3OF9aXNE200PMIaH9o6W8YE6spAPLAOPzrANx_pn90/exec";

  var forms = document.querySelectorAll("form.contact-form");
  if (!forms.length) return;

  function inviaAGoogleSheet(form, tipo) {
  ...
  forms.forEach(function (form) {
    ...
    form.addEventListener("submit", function (evento) {
      ...
      if (form.id === "review-form") inviaAGoogleSheet(form, "recensione");
      else if (form.id === "contact-form") inviaAGoogleSheet(form, "contatto");
